...
authorize_usb_for_offline_media() {
  ...
  # Helper still on media (running from USB before sbin install)
  local self_dir
  self_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
  if [[ -f "$self_dir/authorize-offline-usb.sh" ]]; then
    log "Running co-located authorize-offline-usb.sh from media"
    bash "$self_dir/authorize-offline-usb.sh" || true
    return 0
  fi
  log "USB authorize (inline last-resort): stop USBGuard, authorize devices, load hid/storage"
...
write_repo_file() {
  ...
  local epel_en=0 fusion_en=0 rke2_en=0
  if [[ -d "$base/EPEL/repodata" || -d "$base/EPEL/Packages" ]]; then
    epel_en=1
  fi
  if [[ -d "$base/RPMFusion/repodata" || -d "$base/RPMFusion/Packages" ]]; then
    fusion_en=1
  fi
  if [[ -d "$base/RKE2/repodata" || -d "$base/RKE2/Packages" ]]; then
    rke2_en=1
  fi
  cat > "$out" <<EOF
# Generated by airgap-setup-1-copy-mirror.sh on $(date -Is)
[${prefix}-baseos]
...
gpgcheck=0
[${prefix}-rpmfusion]
name=Offline RPM Fusion (${base})
baseurl=file://${base}/RPMFusion
enabled=${fusion_en}
gpgcheck=0
[${prefix}-rke2]
name=Offline RKE2 (${base})
...
install_helpers_from() {
  ...
  if [[ -x "$src/scripts/install-airgap-helpers.sh" ]]; then
    ...
    # Last resort: install-airgap-helpers.sh missing on media — minimal best-effort
    # copy driven by the canonical script list, docs to /root/airgap-docs only.
    log "WARN: install-airgap-helpers.sh not found; doing minimal helper copy from $src"
    mkdir -p /usr/local/sbin /root/airgap-docs
    local s list="$src/scripts/target-scripts.list"
    if [[ -f "$list" ]]; then
      while IFS= read -r s || [[ -n "$s" ]]; do
        s="${s%%#*}"; s="$(echo "$s" | tr -d '[:space:]')"
        [[ -n "$s" && -f "$src/scripts/$s" ]] && cp -a "$src/scripts/$s" /usr/local/sbin/ && chmod 755 "/usr/local/sbin/$s"
      done < "$list"
    fi
